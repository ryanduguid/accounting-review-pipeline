...
class TestPaydaySuperRules(unittest.TestCase):
    def test_sg_statutory_rate_is_12_percent(self) -> None:
        ...
        with open(payroll_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                q_earnings = Decimal(row["QualifyingEarnings_CodeQ"])
                liability = Decimal(row["SuperLiability_CodeL"])
                expected_liability = round(q_earnings * Decimal("0.12"), 2)
    ...
    def test_sg_charge_has_the_four_payday_super_components(self) -> None:
        ...
        for row in payroll_rows():
            ...
            base_shortfall = Decimal(row["SuperLiability_CodeL"])
    ...
    def test_received_contribution_reduces_the_final_shortfall_to_nil(self) -> None:
        ...
        for row in payroll_rows():
            ...
            checked += 1
            base_shortfall = Decimal(row["SuperLiability_CodeL"])
            charge = Decimal(row["SGC_Shortfall"])
            self.assertLess(
                charge,
                base_shortfall,
                msg=(
            ...
            self.assertGreater(charge, Decimal("0.0"), f"Event {row['EventID']} is late so the charge cannot be nil")
        self.assertGreater(checked, 0, "Fixture has no LATE_BREACH events to check")
    ...
    def test_a_contribution_received_by_the_due_day_is_not_offset(self) -> None:
        ...
        self.assertEqual(notional_earnings(Decimal("1000.0"), due, due), Decimal("0.0"))
        self.assertEqual(notional_earnings(Decimal("1000.0"), due, due - datetime.timedelta(days=3)), Decimal("0.0"))
        self.assertGreater(notional_earnings(Decimal("1000.0"), due, due + datetime.timedelta(days=1)), Decimal("0.0"))
    ...
    def test_notional_earnings_use_each_days_own_rate_across_a_quarter_boundary(self) -> None:
        ...
        try:
            GIC_PUBLISHED_RATES[(2026, 4)] = Decimal("0.2000")  # deliberately distinct from Q3
            ...
            while day <= received:
                rate = Decimal("0.1143") if day.month <= 9 else Decimal("0.2000")
                balance += balance * (rate / gic_days_in_year(day.year))
                day += datetime.timedelta(days=1)
            expected = round(balance - Decimal("1000.0"), 2)

            self.assertAlmostEqual(notional_earnings(Decimal("1000.0"), due, received), expected, places=2)

            # A single-rate model would differ, which is the defect this guards.
            flat_daily = Decimal("0.1143") / gic_days_in_year(2026)
            flat = round(Decimal("1000.0") * ((1 + flat_daily) ** (received - due).days - 1), 2)
    ...
    def test_compliance_status_flagging(self) -> None:
        ...
        with open(payroll_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                sgc = Decimal(row["SGC_Shortfall"])

                if rec_date > due_date:
                    self.assertEqual(status, "LATE_BREACH", f"Event {row['EventID']} should be flagged LATE_BREACH")
                    self.assertGreater(sgc, Decimal("0.0"), f"Late event {row['EventID']} must have non-zero SGC shortfall")
                else:
                    self.assertEqual(status, "ON_TIME", f"Event {row['EventID']} should be flagged ON_TIME")
                    self.assertEqual(sgc, Decimal("0.0"), f"On-time event {row['EventID']} must have 0.0 SGC shortfall")
