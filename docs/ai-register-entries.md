...
Each entry describes the version named in its System row, as that version's README and tests describe it. Record the installed version in the firm's register, and recheck the entry against the component's README when the two differ. An entry is not a certification, an approval of any use or a statement that a firm's use complies with anything. The firm's own AI-use policy decides what may be done with client data.
...
| For the firm to complete | Owner, installed version, approved uses, policy sign-off date, approved model destinations, next review date |
...
| For the firm to complete | Owner, installed version, whether any non-synthetic use is approved, next review date |

## Workpaper Review Gate

| Field | Entry |
| --- | --- |
| System | [Workpaper Review Gate](../packages/review-ready-gate/README.md) (`review-ready-gate`) 0.1.8, a readiness gate for workpaper packs before manager review |
...
| Data handled | Workpaper artefacts (CSV and JSON) in a separate working directory and, with `--document`, an original PDF or text document with its extracted text and an intake manifest of proposed fields and human corrections |
| Network access | None |
...
| For the firm to complete | Owner, installed version, engagement types it gates, tolerance policy, next review date |
