...
| Catherby Fisheries Pty Ltd | `contracts/xero-trial-balance-v1/fixtures/` (`passing.csv`, `failing_movement.csv`, `failing_ytd.csv`), `expected_results.json`, `SHA256SUMS`; `packages/xero-trial-balance-export/samples/sample-output.csv`, `assets/quick-proof.*`, exporter tests | Report date 30 June 2026, GUID `AccountID`s and text `AccountCode` (`090`); 2 accounts in the contract fixtures and 10 in the exporter sample; movement 1,200.00 and YTD 15,234.50, one cent breaks in the 2 failing files; exit codes and output markers per scenario | Exporter evaluation and quick proof; conformance tests in all 3 review packages, the Excel adapter, Power BI and the root joined test; the site's exporter page and refusals page |
...
payroll events) and it is already the tenant in 2 review packages. Add one
member and 3 columns:
...
| ENT002 | Draynor Produce Pty Ltd | Trading subsidiary, wholesale | VIC | accruals, quarterly | The Harbour Light story: North and South tracking, 5 Victorian roles, nil payroll tax |
...
Creating the directory also amends the 3 files that name the v1
...
4. BAS skill. `bas-preparation` takes the ENT001 pack, the prior quarter's
   ...
   regenerated from it and the 2 Cedar and Pine fact sets collapse into
   one.
...
| --- | --- | --- | --- |
| Cedar and Pine Consulting | ENT001 Varrock Ventures | Regenerate the skills README preview from the ENT001 September 2026 pack; add firm-based Workpaper Review Gate examples beside the existing 3 | The existing `bas-*` examples are pinned by `test_loader.py`, `test_engine.py` and the site's refusals page at `v0.1.3`; remove them only in a later minor release, in their own pull request |
| Harbour Light | ENT002 Draynor Produce, VIC | Regenerate `data/xero_pl.csv`, `xero_bs.csv` and `xero_pl_tracking.csv` from the ENT002 July 2026 ledger; rename the example directory, intake, profile and registries; update the 9 tests to the firm figures | au-fpa-pack 0.1.1 is published on PyPI; preserve its distribution identity, the 13-week window and the 28 October BAS due date |
...
| Varrock group (Power BI) | The base of the firm | Retire `generate_fixtures.py` in favour of the root generator; the 6 samples become copies with a drift test against `contracts/fabricated-firm-v1/`; add ENT005 and the 3 columns; update the `Dim_Entity` width | `Fx_ValidateABN` must keep passing, so ENT005 needs a checksum-valid ABN |
...
3. Whether the 5 ABNs are checked by hand against ABR Lookup so that no
...
5. Coverage from 1 July 2024 (3 years, matching the ledger) or FY2027
...
6. The generator's home: the root `tools/fabricated_firm.py` proposed in
   ...
   component the author of a contract 3 review packages consume.
