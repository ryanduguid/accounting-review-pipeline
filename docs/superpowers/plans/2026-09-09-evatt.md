...
Note: `[project.scripts]` points at `evatt.cli:main`, which does not exist until Task 8, and `readme = "README.md"` names a file Task 9 creates. Both are fine for `pytest`, `ruff`, `mypy` and `uv lock`. Neither is fine for `python -m build`, which first runs in Task 9; the clean-wheel smoke first runs in Task 10.
...
### What Task 2 actually shipped

The code block above is what was dispatched. Three fix rounds changed it materially, and the git history is the record rather than a re-transcription here. Commits `a354048`, `5cef400`, `aaf77ee`, `a1c192d`.

- A labelled identifier is admitted on the label alone, with no check digit, matching the origin module's deliberate decision that the brief had reversed. Labelled patterns were added for ABN, ACN and Medicare, which had none.
- The twelve month names left the statutory word list, because they suppressed real people (June Smith, April Jones, Ray May).
- All four bare patterns carry the one-character money guard only, so identifiers in table rows and dated prose are detected.
- `DOB` accepts `dd/mm/yyyy` and `dd-mm-yyyy`; `ADDRESS` accepts six more street types; `PLACEHOLDER` gained a left boundary.
- Tests grew from 16 to 33, including one pinning the non-backtracking separator form and one pinning the sorted, non-overlapping and slice-back guarantees Task 4 relies on.
...
Do NOT run `python -m build` here. `pyproject.toml` declares `readme = "README.md"` and that file is not created until Task 9, so the build fails until then. The first build runs in Task 9.
...
this package supports is narrower: a recipient who does not receive the map
cannot use it to reverse named-entity placeholders.
...
- [ ] **Step 5: Run the linters and the first build**
...
Expected: linters clean, wheel and sdist in `dist/`.

This is the first `python -m build` in the plan. It could not run earlier because `pyproject.toml` declares `readme = "README.md"` and this task creates that file.
