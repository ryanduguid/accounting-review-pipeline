...
7. Fetch current remote `main`, create a namespaced annotated tag on that exact
   commit, for example `git tag -a xero-trial-balance-export/v0.1.11 -m "xero-trial-balance-export v0.1.11"`
...
```bash
tag=xero-trial-balance-export/v0.1.11
...
policy_sha="$(gh api -H "Accept: application/vnd.github.raw+json" \
  "repos/$repo/contents/.github/workflows/release-xero-trial-balance-export.yml?ref=$release_commit" \
