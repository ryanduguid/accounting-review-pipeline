...
Every export validates the response before replacing an existing CSV or manifest. It requires exactly one report identified as `TrialBalance`, the expected columns, and unique, non-empty account IDs. A repeated column title is rejected before any rows are read. Its single `As at D Month YYYY` title must match the requested date, using a full English month name. Xero's separate `ReportDate` is not used as the reporting period; the [documented example](https://developer.xero.com/documentation/api/accounting/reports#trial-balance) gives those dates different values.
