...
5. Create the annotated component tag on current remote `main`, for example `git tag -a monthly-close-control-plane/v0.1.6 -m "monthly-close-control-plane/v0.1.6"` (or `-s` when signing is configured), then push only that tag.
...
```bash
tag=monthly-close-control-plane/v0.1.6
repo=ryanduguid/accounting-review-pipeline
