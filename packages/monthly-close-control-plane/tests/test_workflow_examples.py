...
def test_repository_versions_and_yaml_dev_dependency_agree() -> None:
    ...
    assert project_version == lock_version == release_version == "0.1.5"
...
def test_repository_identity_is_distinct_from_package_identity() -> None:
    ...
    assert project["project"]["urls"]["Documentation"] == (
        f"{REPOSITORY_URL}/tree/monthly-close-control-plane/v0.1.5/"
    ...
    assert (
        ...
        "`close-control` and `openaccountants-au` | 0.1.5 |"
    ) in root_readme
...
def test_release_guidance_uses_the_namespaced_current_tag() -> None:
    ...
    assert "monthly-close-control-plane/v0.1.5" in guidance
    assert "tag=monthly-close-control-plane/v0.1.5" in guidance
