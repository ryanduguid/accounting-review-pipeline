...
close-control drivers --pack-dir ../close-demo/pack \
  ...
  --top 5 --output ../close-demo/drivers
```
...
`Credit` with at most two decimal places. A repeated `TransactionID` for the
...
transaction file is incomplete or covers a different window. Across a 30 June
reset, profit-and-loss YTD figures restart, so the movement and the window's
transactions are not comparable for those accounts.
