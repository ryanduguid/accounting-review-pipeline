...
5. Create the annotated component tag on current remote `main`, for example `git tag -a elizabeth-anne-alexander/v0.2.4 -m "elizabeth-anne-alexander/v0.2.4"` (or `-s` when signing is configured), then push only that tag.
...
```bash
tag=elizabeth-anne-alexander/v0.2.4
...
policy_sha="$(gh api -H "Accept: application/vnd.github.raw+json" \
  "repos/$repo/contents/.github/workflows/release-elizabeth-anne-alexander.yml?ref=$release_commit" \
