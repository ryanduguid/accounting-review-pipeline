#!/usr/bin/env python3
"""
End-to-End Monthly Review Pipeline:
Connects Xero Trial Balance Export -> Monthly Close Control Plane -> ATO Benchmark Review.

Usage:
    python pipeline_monthly_review.py [--tb path/to/trial_balance.csv] [--output report.md]
"""
...
def run_integrity_checks(rows: List[AccountRow]) -> Dict[str, Any]:
    ...
    # Abnormal balance checks (e.g. Asset with net Credit, Liability with net Debit)
    exceptions: List[str] = []
    for r in rows:
        sec = r.section.lower()
...
def calculate_ato_benchmarks(rows: List[AccountRow]) -> Dict[str, Any]:
    ...
    for r in rows:
        ...
        # Revenue / Turnover (Credits in Revenue)
...
def generate_markdown_report(tb_path: Path, integrity: Dict[str, Any], benchmarks: Dict[str, Any], rows: List[AccountRow]) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="End-to-End Monthly Close & Benchmark Review Pipeline")
    parser.add_argument("--tb", type=str, default="examples/current_trial_balance.csv", help="Path to trial balance CSV")
    parser.add_argument("-o", "--output", type=str, default="monthly_review_packet.md", help="Path to output Markdown report")
    args = parser.parse_args()

    tb_path = Path(args.tb)
    if not tb_path.exists():
        # Fallback to local sample if not found
        sample_path = Path(__file__).resolve().parent / "current_trial_balance.csv"
        if sample_path.exists():
            tb_path = sample_path
        else:
            print(f"Error: Trial balance file {args.tb} not found.")
            sys.exit(1)

    print(f"Ingesting trial balance: {tb_path}")
    rows = parse_trial_balance(tb_path)
    print(f"Loaded {len(rows)} accounts.")

    print("Running control plane integrity checks...")
    integrity = run_integrity_checks(rows)

    print("Calculating ATO small business benchmark ratios...")
    benchmarks = calculate_ato_benchmarks(rows)

    print(f"Generating review report -> {args.output}")
    report_md = generate_markdown_report(tb_path, integrity, benchmarks, rows)

    out_path = Path(args.output)
    out_path.write_text(report_md, encoding="utf-8")
    print("Review packet generation complete.")
